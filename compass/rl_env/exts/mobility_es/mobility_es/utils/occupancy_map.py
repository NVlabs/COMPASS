...
class OccupancyMapCollisionChecker:
    """Convert a occupancy map to grid map for collision checking.
        Note: This collision checker only works when the following conditions are met:
            ...
            3. Existing bundled COMPASS maps use a top-left YAML origin. Auto-discovered
               maps generated by scripts/generate_omap_from_usd.py use Isaac Sim's ROS
               bottom-left YAML origin.
    """
    ...
    def __init__(self, cfg, precompute_valid_poses=False):    # pylint: disable=unused-argument
        ...
        # Precomputed valid pose storage (populated lazily by precompute_valid_poses()).
        ...
        if yaml_file_path is None:
            ...
            if isinstance(usd_path, str) and usd_path:
                ...
                if os.path.exists(candidate):
                    ...
                    self.__origin_convention = "bottom-left"
    ...
    def _load_grid_map(self, yaml_file_path):
        ...
        img = np.array(img)
        img_data = img[:, :, 0] if img.ndim == 3 else img
