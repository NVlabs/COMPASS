...
class OccupancyMapCollisionChecker:
    ...
    def __init__(self, cfg, precompute_valid_poses=False):    # pylint: disable=unused-argument
        ...
        self.__origin_convention = 'top-left'    # Default: COMPASS original convention
        ...
        for key, val in OMAP_PATHS.items():
            if env_prim_path.endswith(key):
                # Support both string (path only) and dict (path + origin_convention)
                if isinstance(val, dict):
                    yaml_file_path = val["path"]
                    self.__origin_convention = val.get("origin_convention", "top-left")
                else:
                    # String path - default to 'top-left' convention
                    yaml_file_path = val
                    self.__origin_convention = "top-left"
                break

        if yaml_file_path and os.path.exists(yaml_file_path):
            print(f'Loaded omap for env {env_prim_path} (origin convention: {self.__origin_convention})')
    ...
    def _load_grid_map(self, yaml_file_path):
        ...
        if self.__map_meta["negate"]:
            res = np.where((img_data / 255) > self.__map_meta["free_thresh"])
        else:
            res = np.where(((255 - img_data) / 255) > self.__map_meta["free_thresh"])
    ...
    def __transform_to_image_coordinates(self, point):
        p_x = point[:, 0]
        p_y = point[:, 1]
        # NOTE: World frame has x facing right, y facing up.
        # Image frame has x (column) facing right, y (row) facing down (top-left = (0,0)).
        resolution = self.__map_meta["resolution"]
        i_x = torch.floor(
            (p_x - self.__map_meta["origin"][0]) / resolution).reshape(
                (point.shape[0], 1))

        # Y coordinate transformation depends on origin convention
        if self.__origin_convention == 'bottom-left':
            # ROS convention: origin[1] is bottom-left Y in world
            # Image row 0 (top) corresponds to world Y = origin[1] + height_meters
            height_pixels = self.__grid_map.shape[0]
            ...
            # COMPASS convention: origin[1] is top-left Y in world
            i_y = torch.floor(
    ...
    def precompute_valid_poses(self, start_collision_distance, goal_collision_distance,
        ...
        # Precompute valid orientations for each pose location only if requested
        if precompute_valid_orientations:
            ...
            self.__goal_pose_valid_orientations = None
    ...
    def _buffer_obstacles(self, grid_map, collision_distance_meters):
        ...
        """
        # Lazy import to avoid importing scipy at module load time, which conflicts
        # with Isaac Sim's bundled scipy/numpy versions loaded during AppLauncher init.
    ...
    def _extract_free_space_locations(self, grid_map, map_meta):
        ...
        if self.__origin_convention == 'bottom-left':
            # ROS convention: origin[1] is bottom-left Y in world
            # Image row 0 (top) -> world Y = origin[1] + height_meters
            # Image row (height-1) (bottom) -> world Y = origin[1]
