...
# Runtime args when launching Isaac Sim with a NuRec USD that requires SPG runtime.
...
# Camera renderer settings for Isaac RTX.
ISAAC_RTX_CAMERA_RENDERER_SETTINGS = {
    "enable_dl_denoiser": True,
    "antialiasing_mode": "DLSS",
}

# Additional camera renderer settings for NuRec USDs that require SPG runtime.
NUREC_SPG_CAMERA_RENDERER_CARB_SETTINGS = {
...
# RenderProduct defaults for kit viewport when using NuRec USDs with SPG runtime.
...
def apply_isaac_rtx_camera_renderer_settings(env_cfg, isaac_rtx, spg_runtime):
    ...
    if getattr(renderer_cfg, "renderer_type", None) != "isaac_rtx":
        renderer_cfg = IsaacRtxRendererCfg(global_settings=IsaacRtxRendererGlobalSettingsCfg())
        camera_cfg.renderer_cfg = renderer_cfg

    global_settings = getattr(renderer_cfg, "global_settings", None)
    if global_settings is None:
        return

    for name, value in ISAAC_RTX_CAMERA_RENDERER_SETTINGS.items():
        setattr(global_settings, name, value)

    if not spg_runtime:
        return

    carb_settings = dict(getattr(global_settings, "carb_settings", None) or {})
    carb_settings.update(NUREC_SPG_CAMERA_RENDERER_CARB_SETTINGS)
    global_settings.carb_settings = carb_settings
...
def configure_nurec_isaacsim_rtx_viewport(nurec_usd_path, spg_runtime, quiet=False):
