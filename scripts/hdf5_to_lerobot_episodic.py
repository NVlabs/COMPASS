    ...
    with h5py.File(hdf5_path, 'r') as f:
        ...
        def visit_func(name, obj):
            if isinstance(obj, h5py.Dataset):
                structure[name] = {'shape': obj.shape, 'dtype': str(obj.dtype), 'size': obj.size}
...
def create_directory_structure(output_path: str,
                               video_key: str = "observation.images.ego_view") -> None:
...
def create_modality_json(state_keys: List[str],
...
def create_info_json(total_episodes: int,
                     total_frames: int,
                     state_dim: int,
                     action_dim: int,
                     fps: float = 20.0,
                     image_shape: Tuple[int, int, int] = (320, 512, 3),
                     video_key: str = "observation.images.ego_view") -> Dict[str, Any]:
...
def convert_episodic_hdf5_to_lerobot(hdf5_paths: List[str],
                                     output_path: str,
    ...
    for chunk_idx, hdf5_path in enumerate(tqdm(hdf5_paths, desc="Processing HDF5 files")):
        ...
        with h5py.File(hdf5_path, 'r') as f:
            ...
            if len(valid_episode_indices) == 0:
                logger.info(
                    f"No episodes with goal_reached=True in chunk {chunk_idx}, skipping chunk.")
                continue
            ...
            for local_episode_idx in tqdm(range(T),
                                          desc=f"Processing episodes in chunk {chunk_idx}",
                                          leave=False):
                ...
                episode_state = np.concatenate(
                    [episode_speeds, episode_routes_flat, episode_goal_headings],
                    axis=1)    # [S, 43]
                ...
                # Core data
                    "observation.state":
                        episode_state.tolist(),    # Concatenated state array per modality.json
                    "action":
                        episode_actions.tolist(),    # Concatenated action array per modality.json
                    "timestamp": timestamps.tolist(),    # Timestamp from episode start

                # Annotation system - indices to meta/tasks.jsonl
                    "annotation.human.action.task_description":
                        [0] * S,    # Points to task_index 0 (main task)
                ...
                # Episode tracking
                    ...
                    "index": list(range(global_observation_idx,
                                        global_observation_idx + S)),    # GLOBAL obs index
                }
                ...
                parquet_path = Path(
                    output_path) / "data" / chunk_name / f"episode_{global_episode_idx:06d}.parquet"
                df.to_parquet(parquet_path, index=False)

                # Save video in the appropriate chunk directory
                video_path = Path(
                    output_path
                ) / "videos" / chunk_name / video_key / f"episode_{global_episode_idx:06d}.mp4"
    ...
    info_data = create_info_json(total_episodes, total_frames, state_dim, action_dim, fps,
                                 image_shape, video_key)
    ...
    logger.info(
        f"✅ Successfully converted {len(hdf5_paths)} HDF5 files to LeRobot format at {output_path}")
...
def main():
    parser = argparse.ArgumentParser(
        description=
        "Convert episodic HDF5 data [T, S, ...] to LeRobot format with multi-chunk support")
    parser.add_argument("--hdf5-dir",
                        type=str,
                        default=None,
                        help="Optional directory to search for all HDF5 files to process")
    parser.add_argument("--hdf5-paths",
                        nargs='*',
                        help="Paths to HDF5 files (each becomes a chunk)")
    ...
    parser.add_argument("--goal-heading-key",
                        default="goal_heading",
                        help="Key for goal heading data in HDF5")
    parser.add_argument("--action-key", default="action", help="Key for action data in HDF5")
    parser.add_argument("--task-description",
                        default="Robot navigation task",
                        help="Description of the task")
    parser.add_argument("--fps",
                        type=float,
                        default=20.0,
                        help="Frames per second for video encoding")
    parser.add_argument("--video-key",
                        default="observation.images.ego_view",
                        help="Output video key name")
    parser.add_argument("--inspect-only",
                        action='store_true',
                        help="Only inspect HDF5 structure without conversion")
    ...
    convert_episodic_hdf5_to_lerobot(hdf5_paths, args.output_path, args.image_key, args.speed_key,
                                     args.route_key, args.goal_heading_key, args.action_key,
                                     args.task_description, args.fps, args.video_key)
